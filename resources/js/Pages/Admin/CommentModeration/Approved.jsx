...
export default function CommentModerationApproved({ auth, approvedComments, counts }) {
    const { t } = useTranslation();
    const [searchTerm, setSearchTerm] = useState('');

    // Check if approvedComments has the expected structure
    const hasData = approvedComments && approvedComments.data;

    // Use empty array as fallback if data is not available
    const commentsData = hasData ? approvedComments.data : [];
    ...
    return (
        <AdminLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">{t('admin.comment_moderation.approved_comments')}</h2>}
        >
            <Head title={t('admin.comment_moderation.approved_comments')} />

            <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div className="p-6 flex flex-col md:flex-row md:items-center md:justify-between">
                    <div className="mb-4 md:mb-0">
                        <h3 className="text-lg font-medium text-gray-900">{t('admin.comment_moderation.approved_comments')}</h3>
                        <p className="mt-1 text-sm text-gray-600">
                            {t('admin.comment_moderation.approved_description')}
                        </p>
                    </div>
                ...
                <div className="p-6">
                    {filteredComments.length > 0 ? (
                        ...
                        <div className="text-center py-4">
                            <p className="text-gray-500">{t('admin.comment_moderation.no_approved_comments')}</p>
                        </div>
                    )}

                    <div className="mt-6">
                        {approvedComments && approvedComments.links && (
                            <Pagination links={approvedComments.links} />
                        )}
                    </div>
                </div>
            </div>
