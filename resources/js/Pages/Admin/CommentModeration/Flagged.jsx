...
export default function CommentModerationFlagged({ auth, flaggedComments, counts }) {
    const { t } = useTranslation();
    const [searchTerm, setSearchTerm] = useState('');

    // Check if flaggedComments has the expected structure
    const hasData = flaggedComments && flaggedComments.data;

    // Use empty array as fallback if data is not available
    const commentsData = hasData ? flaggedComments.data : [];
    ...
    const filteredComments = commentsData.filter(comment =>
        ...
        (comment.flag_reason && comment.flag_reason.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <AdminLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">{t('admin.comment_moderation.flagged_comments')}</h2>}
        >
            <Head title={t('admin.comment_moderation.flagged_comments')} />

            <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div className="p-6 flex flex-col md:flex-row md:items-center md:justify-between">
                    <div className="mb-4 md:mb-0">
                        <h3 className="text-lg font-medium text-gray-900">{t('admin.comment_moderation.flagged_comments')}</h3>
                        <p className="mt-1 text-sm text-gray-600">
                            {t('admin.comment_moderation.flagged_description')}
                        </p>
                    </div>
                ...
                <div className="p-6">
                    {filteredComments.length > 0 ? (
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {t('admin.comment_moderation.flag_reason')}
                                        </th>
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        ...
                                        <tr key={comment.id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-normal">
                                                <div className="text-sm text-red-600">
                                                    {truncateText(comment.flag_reason)}
                                                </div>
                                            </td>
                        ...
                        <div className="text-center py-4">
                            <p className="text-gray-500">{t('admin.comment_moderation.no_flagged_comments')}</p>
                        </div>
                    )}

                    <div className="mt-6">
                        {flaggedComments && flaggedComments.links && (
                            <Pagination links={flaggedComments.links} />
                        )}
                    </div>
                </div>
            </div>
