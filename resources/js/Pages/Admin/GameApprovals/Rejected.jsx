...
export default function GameApprovalRejected({ auth, rejectedGames }) {
    const { t } = useTranslation();
    const [searchTerm, setSearchTerm] = useState('');

    const filteredGames = rejectedGames.data.filter(game => 
    ...
    return (
        <AdminLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">{t('admin.game_approval.rejected_games')}</h2>}
        >
            <Head title={t('admin.game_approval.rejected_games')} />
            ...
            <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div className="p-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">{t('admin.game_approval.rejected_games')}</h3>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {t('admin.game_approval.rejection_date')}
                                        </th>
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {t('admin.game_approval.feedback')}
                                        </th>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {filteredGames.map((game) => (
                                        <tr key={game.id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-900">{formatDate(game.rejected_at)}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="text-sm text-gray-900">
                                                    {game.feedback 
                                                        ? (game.feedback.length > 50 ? game.feedback.substring(0, 50) + '...' : game.feedback)
                                                        : t('admin.game_details.no_feedback')
                                                    }
                                                </div>
                                            </td>
                    ...
                    <div className="mt-6">
                        <Pagination links={rejectedGames.links} />
                    </div>
                </div>
            </div>
        </AdminLayout>
