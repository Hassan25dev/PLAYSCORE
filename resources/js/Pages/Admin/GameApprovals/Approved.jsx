...
export default function GameApprovalApproved({ auth, approvedGames }) {
    const { t } = useTranslation();
    const [searchTerm, setSearchTerm] = useState('');

    const filteredGames = approvedGames.data.filter(game => 
    ...
    return (
        <AdminLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">{t('admin.game_approval.approved_games')}</h2>}
        >
            <Head title={t('admin.game_approval.approved_games')} />
            ...
            <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div className="p-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">{t('admin.game_approval.approved_games')}</h3>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {t('admin.game_approval.approval_date')}
                                        </th>
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {t('admin.game_approval.approved_by')}
                                        </th>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {filteredGames.map((game) => (
                                        <tr key={game.id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-900">{formatDate(game.approved_at)}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-900">{game.approved_by_user ? game.approved_by_user.name : 'System'}</div>
                                            </td>
                    ...
                    <div className="mt-6">
                        <Pagination links={approvedGames.links} />
                    </div>
                </div>
            </div>
        </AdminLayout>
