...
export default function Edit({ auth, jeu, platforms, genres, tags, selectedPlatforms, selectedGenres, selectedTags }) {
    const { t } = useTranslation();
    const [currentStep, setCurrentStep] = useState(1);
    const [formSubmitted, setFormSubmitted] = useState(false);
    const [stepValidationErrors, setStepValidationErrors] = useState({});
    const [previewImage, setPreviewImage] = useState(jeu.image_arriere_plan ? `/storage/${jeu.image_arriere_plan}` : null);
    const [videoPreview, setVideoPreview] = useState(jeu.video_path ? `/storage/${jeu.video_path}` : (jeu.video_url || null));
    ...
    const { data, setData, post, processing, errors, reset, setError, clearErrors } = useForm({
        ...
        date_sortie: formatDateForInput(jeu.date_sortie) || '',
        image_arriere_plan: null, // Will be set if user uploads a new image
        video: null, // Will be set if user uploads a new video
        plateforme_ids: selectedPlatforms || [],
    ...
    const stepValidationRules = {
        ...
        2: () => {
            let stepErrors = {};
            if (!jeu.image_arriere_plan && !data.image_arriere_plan && !previewImage) {
    ...
    const handleSubmit = (submitForApproval = false) => {
        ...
        // Get submit value
        const submitValue = submitForApproval;
        ...
        setTimeout(() => {
            ...
            // Submit the form
            post(route('game-submissions.update', jeu.id), {
                forceFormData: true,
                onSuccess: () => {
                    // Don't reset the form on success for edit
            ...
            case 1:
                return (
                    <BasicInfoStep
                        ...
                        rawgEnabled={false} // Disable RAWG import for editing
                    />
                );
            case 2:
                return (
    ...
    const pageTitle = jeu.statut === 'rejete'
        ? t('developer.submission.resubmit')
        : t('developer.submission.edit_submission');
    ...
    return (
        ...
        >
            ...
            <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div className="p-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">{pageTitle}</h3>

                    {jeu.statut === 'rejete' && jeu.feedback && (
                        <div className="bg-red-50 p-4 rounded-md mb-6">
                            <h4 className="font-medium text-red-800 mb-2">{t('developer.submission.rejection_feedback')}</h4>
